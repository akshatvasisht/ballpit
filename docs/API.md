# Ballpit API Documentation

This document describes the REST API endpoints and smart contract interface for the Ballpit blockchain-based shareholder voting system.


## Overview

The Ballpit API is organized into three functional areas:

1. **KYC Endpoints** (`/api/kyc/*`) - Identity verification and session management
2. **Company Endpoints** (`/api/company/*`) - Admin operations for token minting and vote creation
3. **User Endpoints** (`/api/user/*`) - Shareholder operations for voting and delegation

All company endpoints require admin authentication via the `x-admin-key` header. User endpoints are public but require wallet signatures for on-chain transactions.
...
Every request to the API is assigned a unique `X-Request-ID`. This ID is returned in the response headers and can be used for tracing logs in the backend.
...
**Usage:** Use this ID when debugging or reporting issues. All backend logs include the request ID for correlation.

### Standard Response Format

All successful responses return a JSON object, usually containing a `requestId` field for traceability.
...
The API follows a standardized error response format across all endpoints:
...
**Common Error Codes:**
- `VALIDATION_ERROR`: Input failed Zod validation (usually includes `details`).
- `BLOCKCHAIN_ERROR`: Failure during Solana on-chain interaction.
- `UNAUTHORIZED`: Missing or invalid authentication.
- `ALREADY_CLAIMED`: Business logic failure (e.g., tokens already claimed).
- `INTERNAL_ERROR`: Unexpected server failure.

### Input Validation
The API uses **Zod** for strict input validation. If a request body fails validation, the API returns a `400 Bad Request` with structured details.
...
```json
{
  "verificationUrl": "http://localhost:3000/verify?sessionId=...",
  "sessionId": "mock_session_123456789"
}
```
...
Checks the verification status of a KYC session. Supports both path parameters and query strings.

**Parameters:**
- `sessionId` (string, path or query): The KYC session ID to check
...
### Batch Mint Share Tokens

**Endpoint:** `POST /api/company/batch-mint`

Mints share tokens to multiple verified shareholders in a single request.
...
**Status Codes:**
- `200`: Batch mint completed (check results array for individual status)
- `400`: Invalid request body
- `403`: Admin authentication failed
- `500`: Batch minting failed
...
All analytics endpoints require admin authentication via `x-admin-key` header.

### Get Analytics Overview

**Endpoint:** `GET /api/analytics/overview`

Retrieves aggregated participation and proposal metrics for the admin dashboard.
...
```json
{
  "overview": {
    ...
    "closedProposals": 3,
    "totalSupply": 1000,
    "totalVotesCastAcrossAll": 150,
    "aggregateParticipationRate": "15.00%"
  }
}
```

...
This document describes the testing approach, standards, and procedures for the Ballpit blockchain-based shareholder voting system.


## Test Coverage Overview

The Ballpit test suite is organized into three layers:

1. **Smart Contract Tests** (`voting_contract/tests/`) - Anchor/TypeScript tests for on-chain voting logic (9 tests)
2. **Backend Integration Tests** (`backend/tests/`) - Jest tests for REST API endpoints and Solana integration (16 tests)
3. **End-to-End Tests** (`backend/e2e/`) - Playwright browser tests for complete shareholder journeys (4 scenarios)

**Total Test Count:** 29+ tests covering smart contracts, API routes, and user flows.
...
**Running Tests:**
Recommended industry-standard commands:
```bash
# From the root directory:
npm run test:contract

# Or from the voting_contract directory:
npm test
```

**Test Suite Overview:**
The suite includes 9 comprehensive test cases covering:
1. **Initialize Vote**: Verification of ballot box creation.
2. **Cast Vote**: Successful shareholder voting.
3. **Double Voting Prevention**: Security check for duplicate votes.
4. **Token Requirements**: Verification that 0-token users cannot vote.
5. **Vote Closing**: Authority-led proposal finalization.
6. **Delegation Setup**: Proxy voting power assignment.
7. **Proxy Voting**: Delegate-led voting on behalf of owners.
8. **Delegation Security**: Verification of delegate authorization.
9. **Delegation Revocation**: Secure cleanup of delegation records.

**Test Structure:**
- Tests are written in TypeScript using Anchor's test framework.
- The `test.sh` script manages the local Solana Agave validator, handles airdrops, and executes the suite.
- Each test run starts with a fresh validator state for determinism.
...
**Test Suite Overview:**
The suite includes **16 passing tests** covering:
- Admin authentication and unauthorized access prevention
- KYC session creation and status polling
- Token minting (standard and batch)
- Vote creation and closing
- Share claiming (with KYC approval checks)
- Vote casting (including insufficient token checks)
- Analytics aggregation


### End-to-End Tests (Playwright)

**Location:** `backend/e2e/e2e.spec.js`

**Running Tests:**
```bash
cd backend
# Install browsers if first time
...
**Test Coverage:**
The E2E suite verifies the **Shareholder Journey**:
1. **Landing**: Navigation to home page and heading verification.
2. **Verification**: Multi-step identity form and document upload.
3. **Approval**: Polking for KYC approval and success page transition.
4. **Wallet**: Selecting a wallet (e.g., Phantom) and finalizing link.
5. **Dashboard**: Transition to the main dashboard.
6. **Actions**: Claiming shares and casting a vote via on-screen actions.

**Test Framework:** Jest with Supertest

**Mocking Strategy:**
- Comprehensive mocks for Solana `@web3.js` and Anchor program methods.
- Mocked SPL Token interactions for minting and account management.
- Mocked Didit KYC and Database layers for deterministic API testing.
